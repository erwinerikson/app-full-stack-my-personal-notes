...
class App extends React.Component {
  ...
  render() {
    ...
    const filteredNotes = searchKeyword === '' ? notes : notes.filter(item =>
    ...
    activeNotes.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    const archivedNotes = filteredNotes.filter(note => note.archived === true);
    archivedNotes.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    return (
      <div className="note-app" data-testid="note-app">
        <div className="note-app__header" data-testid="note-app-header">
          <h1>Notes</h1>
          <NoteSearch changeWord={ this.onSearchHandler } />
        </div>
